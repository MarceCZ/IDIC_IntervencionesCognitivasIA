...
HOST_RESP = 'localhost'  # servidor local que procesa respuesta (Py3)
PORT_SOCKET = 12345
...
def _to_unicode(x):
    """Convierte bytes/str a unicode (Py2 safe)."""
    try:
...
def _parse_model_response(raw_bytes):
    """
    Soporta 2 formatos:
    1) "si" / "no" (legacy)
    2) JSON: {"verdict":"si|no","transcript":"..."}
    Retorna: (verdict_unicode, transcript_unicode)
    """
    if raw_bytes is None:
        return (u"no", u"")

    raw_u = _to_unicode(raw_bytes).strip()

    # Intentar JSON
    if raw_u.startswith(u"{") and raw_u.endswith(u"}"):
        try:
            obj = json.loads(raw_u)
            verdict = _to_unicode(obj.get("verdict", u"no")).strip().lower()
            transcript = _to_unicode(obj.get("transcript", u""))
            if verdict not in (u"si", u"no"):
                verdict = u"no"
    ...
    # Legacy
    low = raw_u.strip().lower()
    if low.startswith(u"si"):
        return (u"si", u"")
    return (u"no", u"")

# ================== VALIDACIÓN MANUAL GUI ==================
def validacion_manual_gui(pregunta, respuesta_estudiante):
    """
    GUI Py2: muestra pregunta + respuesta y devuelve u"si" o u"no".
    Botón 'Correcto' => u"si"
    Botón 'Incorrecto' => u"no"
    Regla: si respuesta es "No respondió" => no (sin GUI).
    """
    pregunta_u = _to_unicode(pregunta)
    resp_u = _to_unicode(respuesta_estudiante).strip()

    # Si no respondió, NO se abre GUI
    if (not resp_u) or (resp_u.lower() == u"no respondió"):
        return u"no"
    ...
    try:
        ...
        print("\n=== VALIDACIÓN MANUAL ===")
        try:
            print("PREGUNTA:", pregunta_u.encode("utf-8"))
            print("RESPUESTA:", resp_u.encode("utf-8"))
        except Exception:
            pass
        val = raw_input("¿Marcar como correcto? (s/n): ").strip().lower()
        return u"si" if val == "s" else u"no"
    ...
    root = tk.Tk()
    root.title("Validación manual (IA dijo: NO)")
    root.resizable(True, True)

    frm = tk.Frame(root, padx=16, pady=16)
    frm.pack(fill="both", expand=True)

    tk.Label(frm, text=u"Pregunta:", font=("Arial", 12, "bold")).pack(anchor="w")

    txtp = tk.Text(frm, height=3, wrap="word", font=("Arial", 12))
    txtp.pack(fill="x", expand=False, pady=(4, 12))
    txtp.insert("1.0", pregunta_u)
    txtp.config(state="disabled")

    tk.Label(frm, text=u"Respuesta del estudiante:", font=("Arial", 12, "bold")).pack(anchor="w")

    txtr = tk.Text(frm, height=8, wrap="word", font=("Arial", 12))
    txtr.pack(fill="both", expand=True, pady=(4, 12))
    txtr.insert("1.0", resp_u if resp_u else u"(vacío / no se pudo transcribir)")
    txtr.config(state="disabled")

    btns = tk.Frame(frm)
    btns.pack(fill="x")

    tk.Button(btns, text="Correcto", font=("Arial", 12), width=12, command=on_correcto)\
        .pack(side="left", padx=(0, 8))
    tk.Button(btns, text="Incorrecto", font=("Arial", 12), width=12, command=on_incorrecto)\
        .pack(side="left")
    ...
    root.mainloop()
    return result["verdict"] if result["verdict"] in (u"si", u"no") else u"no"
...
def procesar_respuesta(texto):
    ...
    try:
        s.sendall(data_out)
        data = s.recv(8192)  # por si viene JSON + transcript
    finally:
        s.close()

    verdict, transcript = _parse_model_response(data)
    return verdict, transcript
...
# ================== GUI "DETENER" GRABACIÓN ==================
def ventana_detener_grabacion(stop_event, done_event, titulo=u"Grabando respuesta..."):
    """
    Ventana Tkinter con botón 'Detener' que:
      - stop_event.set() si el usuario detiene
      - se cierra sola si done_event se activa (grabación terminó por silencio)
    """
    try:
        import Tkinter as tk
    except Exception:
        # Sin GUI => no hacemos nada
        return

    root = tk.Tk()
    root.title(_to_unicode(titulo))
    root.resizable(False, False)

    frame = tk.Frame(root, padx=18, pady=18)
    frame.pack()

    tk.Label(frame, text=u"Grabando... (termina por silencio o presiona Detener)", font=("Arial", 12))\
        .pack(pady=(0, 12))
    ...
    tk.Button(frame, text="Detener", font=("Arial", 12), width=12, command=on_stop).pack()
...
def respuesta_alumno_humano(robot, contexto, log_timeline_filename=None, etiqueta=u"concepto", definicion_concepto=None):
    ...
    Ahora:
      - termina por silencio normal
      - y además puedes presionar "Detener" con GUI.
    Devuelve: (verdict u"si"/u"no", transcript u"...")
    """
    ...
    rec_out = {"first_start": None}

    def _rec_worker():
        try:
            # Si grabar_audio.py no soporta stop_event, esto lanzará TypeError
            rec_out["first_start"] = record_until_silence(
                ip=NAO1_IP,
                port=NAO_PORT,
                out_wav=RUTA_AUDIO,
            ...
            # Fallback: sin stop_event (pero al menos sigue funcionando)
            rec_out["first_start"] = record_until_silence(
                ip=NAO1_IP,
                port=NAO_PORT,
                out_wav=RUTA_AUDIO
    ...
    th = threading.Thread(target=_rec_worker)
    th.daemon = True
    th.start()

    # GUI para detener
    ventana_detener_grabacion(stop_event, done_event, titulo=u"Grabando ({})".format(etiqueta))
    ...
    if log_timeline_filename:
        ...
        if rec_out["first_start"] is not None:
            log_event(log_timeline_filename, u"INICIO voz sujeto ({:.3f}s)".format(float(rec_out["first_start"])))
        else:
            log_event(log_timeline_filename, u"No respondió")

    descargar_audio()

    # Construir contexto enriquecido: pregunta|tipo|definicion
    ...
    verdict, transcript = procesar_respuesta(contexto_socket)
    try:
        print(u"Respuesta del modelo (verdict): {}".format(verdict))
    except Exception:
        pass
...
def fase_observacion(robot1_proxies, robot2_proxies, texto, log_timeline_filename=None):
    ...
    motion1.setAngles(["HeadYaw", "HeadPitch"], [-0.6, 0.1], 0.1)
    motion2.setAngles(["HeadYaw", "HeadPitch"], [0.6, 0.1], 0.1)
...
def preguntar_y_validar(robot, texto, log_timeline_filename=None, etiqueta=u"concepto",
                        log_line_for_question=None, definicion_concepto=None):
    """
    - Si la IA dice "no", se abre validación manual GUI,
      EXCEPTO si transcript es "No respondió".
    - Retorna u"si" o u"no" (flujo igual al tuyo).
    """
    if log_timeline_filename and log_line_for_question:
        log_event(log_timeline_filename, log_line_for_question)

    pregunta = texto.split('|', 1)[0].strip()
    text_temp = "\\vct=95\\\\rspd=90\\" + pregunta
    hablar_con_gestos(robot["animated_speech"], text_temp)

    verdict, transcript = respuesta_alumno_humano(
        robot,
        texto,
    ...
    verdict_u = _to_unicode(verdict).strip().lower()
    transcript_u = _to_unicode(transcript).strip().lower()

    # SOLO validar manualmente si IA dijo NO y SI hubo respuesta
    if verdict_u == u"no" and transcript_u not in (u"", u"no respondió"):
        if log_timeline_filename:
            log_event(log_timeline_filename, u"[MANUAL] IA dijo NO. Abriendo validación manual...")
        manual = validacion_manual_gui(pregunta, transcript)
        if log_timeline_filename:
            log_event(log_timeline_filename, u"[MANUAL] Resultado manual: {}".format(manual))
        verdict_u = manual
...
def fase_experimental(robot1_proxies, robot2_proxies, texto, definicion_concepto, log_timeline_filename=None, pausa_larga=False):
    ...
    if respuesta != u"si":
        ...
        if respuesta != u"si":
            ...
            # 1d) post-modelo: humano responde (y también aplica validación manual dentro de preguntar_y_validar, pero aquí es directo)
            verdict, transcript = respuesta_alumno_humano(
                robot1_proxies,
                pregunta_concepto,
                log_timeline_filename=log_timeline_filename,
            ...
            if verdict_u == u"no" and transcript_u not in (u"", u"no respondió"):
                if log_timeline_filename:
                    log_event(log_timeline_filename, u"[MANUAL] IA dijo NO (post-modelo). Abriendo validación manual...")
                manual = validacion_manual_gui(pregunta_concepto.split('|', 1)[0].strip(), transcript)
                if log_timeline_filename:
                    log_event(log_timeline_filename, u"[MANUAL] Resultado manual (post-modelo): {}".format(manual))
                verdict_u = manual
    ...
    if respuesta != u"si":
        ...
        if verdict_u == u"no" and transcript_u not in (u"", u"no respondió"):
            if log_timeline_filename:
                log_event(log_timeline_filename, u"[MANUAL] IA dijo NO (ejemplo post-modelo). Abriendo validación manual...")
            manual = validacion_manual_gui(pedir_ejemplo.split('|', 1)[0].strip(), transcript)
            if log_timeline_filename:
                log_event(log_timeline_filename, u"[MANUAL] Resultado manual (ejemplo post-modelo): {}".format(manual))
            verdict_u = manual
